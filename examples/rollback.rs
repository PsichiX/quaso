use quaso::{
    ...
    multiplayer::{
        ...
        rollback::{RollbackAuthority, RollbackGameState, RollbackMultiplayer, RollbackPlayerRole},
...
type PlayerRole = RollbackPlayerRole<PLAYER_ROLE, HISTORY_CAPACITY, InputSnapshot, StateSnapshot>;
type Authority = RollbackAuthority<AUTHORITY_CLOCK_CHANNEL>;
type Multiplayer = RollbackMultiplayer<AUTHORITY_CLOCK_CHANNEL>;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    .title("Netcode: Authoritative rollback")
...
impl GameState for State {
    ...
    game_state_custom_event! {
        trait(RollbackGameState)
    }
}

impl RollbackGameState for State {
    ...
    fn time_travel(&mut self, _context: GameContext, target_tick: TimeStamp) {
        for player in self.players.values_mut() {
            player.role.inputs_mut().time_travel_to(target_tick);
    ...
    fn handle_inputs(&mut self, _context: GameContext, current_tick: TimeStamp) {
        ...
        if let Some(player) = self.local_player_mut() {
            let (input_sender, input_history) = match &mut player.role {
                PlayerRole::ServerLocal {
                    input_sender,
                    input_history,
                    ..
                } => (input_sender, input_history),
    ...
    fn find_input_divergence(
        ...
        for player in self.players.values_mut() {
            let (input_receiver, input_history) = match &mut player.role {
