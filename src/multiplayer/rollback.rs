...
pub type RollbackAuthority<const AUTHORITY_CLOCK_CHANNEL: u64> =
    Authority<ClockExtension<AUTHORITY_CLOCK_CHANNEL>>;

pub struct RollbackPrepareFrameEvent {
    pub current_tick: TimeStamp,
}

pub struct RollbackTimeTravelEvent {
    pub target_tick: TimeStamp,
}

pub struct RollbackHandleInputsEvent {
    pub current_tick: TimeStamp,
}

pub struct RollbackFindInputDivergenceEvent {
    pub current_tick: TimeStamp,
    pub out_divergence: Option<TimeStamp>,
}

pub struct RollbackClientFindStateDivergenceEvent {
    pub current_tick: TimeStamp,
    pub out_divergence: Option<TimeStamp>,
}

pub struct RollbackServerSendStateEvent {
    pub current_tick: TimeStamp,
}

pub struct RollbackTickEvent {
    pub current_tick: TimeStamp,
    pub delta_time: f32,
    pub resimulating: bool,
}

pub struct RollbackCommitFrameEvent {
    pub current_tick: TimeStamp,
}

pub trait RollbackGameState {
    ...
    fn custom_event(&mut self, context: GameContext, payload: &mut dyn Any) {
        if let Some(event) = payload.downcast_ref::<RollbackPrepareFrameEvent>() {
...
pub struct RollbackMultiplayer<const AUTHORITY_CLOCK_CHANNEL: u64> {
    pub authority: RollbackAuthority<AUTHORITY_CLOCK_CHANNEL>,
...
impl<const AUTHORITY_CLOCK_CHANNEL: u64> RollbackMultiplayer<AUTHORITY_CLOCK_CHANNEL> {
    pub fn new(network: &GameNetwork) -> Option<Self> {
        ...
        let authority = RollbackAuthority::new(
            is_server,
    ...
    fn maintain_server(&mut self, state: &mut dyn GameState, context: &mut GameContext) {
        let divergence = self.find_input_divergence(state, context);

        if let Some(divergence) = divergence {
            tracing::event!(
                target: "quaso::multiplayer::rollback",
                tracing::Level::DEBUG,
    ...
    fn maintain_client(&mut self, state: &mut dyn GameState, context: &mut GameContext) {
        let input_divergence = self.find_input_divergence(state, context);
        if let Some(input_divergence) = input_divergence {
            tracing::event!(
                target: "quaso::multiplayer::rollback",
                tracing::Level::DEBUG,
                "Input divergence detected at tick {:?}",
        ...
        let state_divergence = self.client_find_state_divergence(state, context);
        if let Some(state_divergence) = state_divergence {
            tracing::event!(
                target: "quaso::multiplayer::rollback",
                tracing::Level::DEBUG,
        ...
        let divergence = TimeStamp::possibly_oldest(input_divergence, state_divergence);
        if let Some(divergence) = divergence {
            self.resimulate(state, context, divergence);
        }
    ...
    fn time_travel(
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackTimeTravelEvent { target_tick };
        state.custom_event(context, &mut payload);
    }
    ...
    fn handle_inputs(&mut self, state: &mut dyn GameState, context: &mut GameContext) {
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackHandleInputsEvent { current_tick };
        state.custom_event(context, &mut payload);
    }
    ...
    fn find_input_divergence(
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackFindInputDivergenceEvent {
            current_tick,
            out_divergence: None,
        };
        state.custom_event(context, &mut payload);
    ...
    fn client_find_state_divergence(
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackClientFindStateDivergenceEvent {
            current_tick,
            out_divergence: None,
        };
        state.custom_event(context, &mut payload);
    ...
    fn server_send_state(&mut self, state: &mut dyn GameState, context: &mut GameContext) {
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackServerSendStateEvent { current_tick };
        state.custom_event(context, &mut payload);
    }
    ...
    fn commit(&mut self, state: &mut dyn GameState, context: &mut GameContext) {
        ...
        let mut context = unsafe { context.fork() };
        context.multiplayer = Some(self);
        let mut payload = RollbackCommitFrameEvent { current_tick };
        state.custom_event(context, &mut payload);
    }
}

impl<const AUTHORITY_CLOCK_CHANNEL: u64> GameMultiplayer
    for RollbackMultiplayer<AUTHORITY_CLOCK_CHANNEL>
{
...
impl<
    ...
    Input: Send + Clone + Serialize + DeserializeOwned + 'static,
    State: Send + Clone + Serialize + DeserializeOwned + 'static,
> RollbackPlayerRole<PEER_ROLE_ID, HISTORY_CAPACITY, Input, State>
{
    ...
    pub fn inputs(&self) -> &HistoryBuffer<Input> {
        match self {
    ...
    pub fn inputs_mut(&mut self) -> &mut HistoryBuffer<Input> {
        match self {
            Self::ServerLocal { input_history, .. } => input_history,
            Self::ClientLocal { input_history, .. } => input_history,
            Self::ServerRemote { input_history, .. } => input_history,
...
impl<
    ...
    Input: Send + Clone + Serialize + DeserializeOwned + 'static,
    State: Send + Clone + Serialize + DeserializeOwned + 'static,
> TypedPeerRole for RollbackPlayerRole<PEER_ROLE_ID, HISTORY_CAPACITY, Input, State>
{
...
impl<
    ...
    Input: Send + Clone + Serialize + DeserializeOwned + 'static,
    State: Send + Clone + Serialize + DeserializeOwned + 'static,
> TypedPeer for RollbackPlayerRole<PEER_ROLE_ID, HISTORY_CAPACITY, Input, State>
{
